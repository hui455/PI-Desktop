...
async function screenshot(name) {
  const shot = await send("Page.captureScreenshot", {
    format: "png",
    // A Windows compositor surface can stop producing frames when occluded.
    // Capture the view while the DOM/lifecycle assertions remain authoritative.
    ...(process.platform === "win32" ? { fromSurface: false } : {}),
  });
