...
import type { Api, AssistantMessage, Context, Model, AssistantMessageEventStream } from "@earendil-works/pi-ai";
...
export class ExtensionModelCompletions {
  ...
  async complete(extensionId: string, model: Pick<Model<Api>, "provider" | "id">,
    ...
    const timer = setTimeout(() => { timedOut = true; controller.abort(); }, options.timeoutMs ?? EXTENSION_COMPLETE_TIMEOUT_MS);
    ...
    const cancel = () => {
      ...
      if (!local) void this.host.call("extensions.model.cancel", { sessionId: this.sessionId, extensionId, requestId: request.requestId })
        .catch(() => undefined);
    };
    controller.signal.addEventListener("abort", cancel, { once: true });
    try {
      const result = local
        ? local.stream(local.model, context, { ...wireOptions, signal: controller.signal }).result()
        : this.host.call<AssistantMessage>("extensions.model.complete", request, EXTENSION_COMPLETE_TIMEOUT_MS + 5000);
